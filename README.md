...
* 자판기에 추가할 상품명과 가격, 수량을 입력받는다.
    * 상품명, 가격, 수량은 쉼표(,)로 구분한다.
        ...
        * 공백값이 들어와서는 안된다.
        * 상품의 정보는 대괄호로 감싸져 있어야한다.
        * 상품의 정보는 이름, 가격, 수량 세개가 모두 들어와야한다.
        * 이미 등록되어 있는 상품을 또 추가할 수 없다.
    * 개별 상품은 세미콜론(;)으로 구분한다.
* 투입 금액을 입력 받는다.
    * 투입금액은 숫자로만 이루어져야한다.
    * 투입금액은 공백이 들어와서는 안된다.
...
* 현재 자판기에 남아있는 금액과 함께 구매할 상품명을 입력받는다.
    * 없는 상품을 입력받으면 예외를 출력한다.
    * 구매할 상품의 가격보다 적은 돈을 갖고있다면 예외를 출력한다.
    * 상품의 수량이 없으면 예외를 출력한다.
    * 입력된 상품의 수량을 하나 차감한다.
    * 구매한 금액만큼 자판기의 투입된 금액이 감소한다.
