...
### 자판기
   1. 초기 자판기 금액 입력 기능
      ...
      - 10원단위가 아닌경우 예외처리
   2. 자판기 보유 동전 생성 기능
      - 500원, 100원, 50원, 10원중 랜덤해서 동전 하나씩 선택하며 가능하다면 넣는 방식으로 구현
      - HashMap 콜렉션에 동전 개수 저장
      - 남은 금액은 모두 10원으로 처리
   3. 현재 자판기 상태 출력 기능
      - 현재 자판기에 동전이 몇개 있는지 출력
   4. 자판기 잔돈 반환 기능
      - 500원, 100원, 50원, 10원 순으로 내려가면서 가능한 최대 개수 고르는 방식으로 구현
   
### 상품
      1. 상품 추가 기능
         - [상품명,가격,갯수];[..] 순으로 잘 입력되었는지 확인
         - 상품명의 경우 첫글자로 공백이 아닌 문자가 나오는지 확인
      
### 사용자
      1. 상품 구매 기능
         - 입력받은 상품이 등록된 상품에 있는지 확인
         - 입력받은 상품의 개수가 0개인지 확인
         - 비어있는 문자열을 입력받았는지 확인
         - 입력받은 상품보다 현재 남은 돈이 작은 경우 확인
         - 모든 상품이 품절인 경우 종료
         - 상품중 가장 싼 가격보다 가진 돈이 작은 경우 종료
      2. 금액 투입 기능
         - 알파벳 등 숫자가 아닌 문자가 입력되어서는 안됨.
         - 음수가 입력되면 안되고, 자연수가 입력되었는지 확인하여야 한다.
