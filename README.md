...
- [x] 상품 구매를 시작한다.
- [x] 남은 금액이 상품의 최저 가격보다 적게 되거나, 모든 상품이 소진되면 구매를 종료한다.
- [x] 구매를 시작할 준비가 되어있는지 확인하고 준비가 되어 있지 않다면 필요한 정보들을 요청한다.
- [x] `투입 금액: %d(현재 투입 금액)원`을 출력한다.
- [x] `구매할 상품명을 입력해 주세요.`를 출력하고 상품명을 입력 받는다.
  - [x] 자판기가 가지고 있는 상품인지 검증한다.
  - [x] 검증에 실패하면 `[ERROR] 존재하지 않는 상품입니다.`를 출력하고 구매 과정을 반복한다.
- [x] 입력 받은 상품을 구매한다. (상품 재고 수량과 남은 돈을 감소시킨다)
- [x] 더 이상 재고가 남지 않은 재품을 목록에서 제거한다.

<br/>
- [x] 최소 개수의 동전으로 잔돈을 돌려준다.
- [x] 각 입력 단계 사이에 빈 줄의 공백을 준다.
