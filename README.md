...
- 자판기 기능
  ...
  - 상품을 추가할 수 있다.
    - 상품명, 가격, 수량을 입력으로 받아 추가한다.
      - 이때, 가격은 100원부터 시작하며, 10원으로 나누어떨어져야 한다.
  - 금액을 투입하면 상품을 구매할 수 있다.
- 사용자의 입력을 검증하는 기능
  - 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException 을 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력후 해당 부분 부터 다시 입력받는다.
    - 검증해야하는 경우
      1. 자판기에서 보유해야하는 금액을 입력 받는 경우
      2. 상품 추가시, 상품명과 가격, 수량을 입력 받는 경우
      3. 사용자가 상품 구매시 투입금액과 상품명을 입력 받는 경우
      4. 남은 돈에 비해 살려고 하는 상품 가격이 더 클 경우 
- 입력을 검증 받고 파싱해주는 기능
  - 상품 추가 입력을 파싱해준다.
  - 간단하게 정수로 파싱해주는 경우는 Integer.parseInt()메서드를 사용한다.
