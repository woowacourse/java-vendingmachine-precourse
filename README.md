...
###1. 자판기 보유 금액 및 동전 개수 설정
- 사용자가 입력한 가격이 10의 배수인지 검증한다.
- 랜덤을 통해 동전 개수를 정한다.
###2. 자판기 상품 설정
- 사용자가 입력한 상품을 ;을 기준으로 분리해야 한다.
- 각 상품마다 ,를 기준으로 분리 후 알맞게 입력됐는지 검증한다.
- 상품명, 가격, 수량이 알맞은 데이터 형식인지 검증한다.
- 상품 금액이 10의 배수인지 검증한다.
###3. 자판기 기능 설정
- 고른 상품이 자판기에 있는지 검증한다.
- 고른 상품이 소진되었는지 검증한다.
- 예외 사항인 경우 에러메세지를 출력하고 상품을 다시 고르게 한다.
