...
## 구현할 기능
- 자판기에 초기 금액을 입력한다.
  - 투입 금액은 0 이상이고 10원 단위로 나누어 떨어져야 한다.
  - 랜덤하게 동전을 만들어준다.
  - 동전이 몇 개씩 있는가를 보여준다.
- 상품들을 입력한다.
  - 상품의 상태가 비어있으면 에러를 만든다.
  - 상품의 이름이 중복되면 에러를 만든다.
  - 상품의 이름이 비어있으면 에러를 만든다.
  - 상품의 가격은 10원 단위로 나누어떨어지고, 0이상이어야 한다. 그렇지 않으면 에러를 만든다.
  - 상품의 개수가 0 이상의 숫자가 아니면 에러를 만든다.
- 투입 금액을 입력한다.
  - 투입 금액은 0 이상이고 10원 단위로 나누어 떨어져야 한다.
  - 투입 금액으로 살 수 있는 물건이 없으면 에러를 만든다.
- 구매할 상품명을 입력한다.
  - 해당 상품이 없으면 에러를 만든다.
  - 해당 상품의 재고가 있는지 확인한다.
    - 재고가 없으면 에러를 만든다.
    - 재고가 있으면 재고를 하나 빼주고, 투입 금액을 갱신한다.
  - 구매 후, 남아 있는 투입 금액으 자판기가 가진 제품 중 살 수 있는 제품이 없으면 프로그램을 종료한다.
  - 구매 후, 모든 상품의 수량이 0이 되면 프로그램을 종료한다.
  - 그 이외의 경우는 다시 상품명을 입력한다.
