...
- [x] 자판기 보유 금액을 입력 받고, 해당 금액으로 최소한의 동전을 생성하는 기능

    ```
    [예외]
    - 입력받은 금액이 숫자가 아니면 예외처리
    - 최소 동전의 단위가 아니면 예외처리 (낙전 발생)
    ```

- [x] 자판기가 보유한 동전을 출력하는 기능
- [x] 상품명과 가격, 수량이 적힌 상품 목록을 입력받고, 상품을 등록하는 기능

    ```
    [예외]
    상품 목록
    - 빈 값이면 예외처리
    - 하나 이상 중복되는 상품명이 있으면 예외처리
    상품
    - 상품 명세가 대괄호([,])로 시작해서 대괄호로 끝나지 않으면 예외처리
    - 상품명과 가격, 수량 세(3) 단위로 나눌 수 없으면 예외처리
    - 가격과 수량이 숫자가 아니면 예외처리
    - 가격이 100원 이상임과 동시에 10원으로 나누어 떨어지지 않으면 예외처리
    - 수량이 0보다 작으면 예외처리
    ```

- [x] 자판기 투입 금액을 입력 받고 설정하는 기능
    ```
    [예외]
    입력받은 금액이 숫자가 아니면 예외처리
    ```
- [x] 구매할 상품명을 입력 받고, 해당 상품을 구매하는 기능

    ```
    [예외]
    - 입력받은 상품명이 존재하지 않는 상품명이면 예외처리
    - 입력받은 상품의 수량이 0개이면 예외처리
    - 투입 금액보다 입력받은 상품의 가격이 더 비싸면 예외처리 (구매가 불가능하므로)
    ```
...
```bash
$ git clone https://github.com/xfrnk2/java-vendingmachine-precourse.git
```
