...
- 자판기가 보유하고 있는 금액 입력
  - 0 이상의 정수만 입력 가능
  - (e) 10으로 나누어 떨어지지 않는 경우

- 자판기가 보유한 동전 계산
  - 동전은 500, 100, 50, 10이 존재
  - 개수 무작위 생성

- 상품명 입력
  - `[상품명,금액,수량],[상품명,금액,수량]` 형식으로 입력
  - (e) 중복된 상품명이 존재하는 경우
  - 상품 금액은 100원 이상
  - (e) 상품 금액이 10으로 나누어 떨어지지 않는 경우
  - 수량은 0 이상 정수만 입력 가능
...
- 구매할 상품명 입력
  - (e) 상품이 없거나 수량이 부족한 경우
  - (e) 투입 금액이 (재고가 존재하는) 상품의 최소 금액보다 적은 경우

- 잔돈 계산
  - 동전 수가 최소가 되도록 구성
  - 잔돈 부족하면 반환 가능한 금액만 반환
  - 잔돈은 동전으로 이루어져 있음
