...
- 사용자로부터 금액을 입력받는다.
- 입력받은 금액이 숫자인지 검증한다.
- 입력받은 금액으로 자판기가 보유한 동전 4가지에 대해 각각 랜덤으로 개수를 생성한다.
- 사용자로부터 구매 할 상품명을 입력받는다. 없는 상품이거나 올바른 이름이 아닐경우 예외처리한다.
- 사용자로부터 자판기의 상품을 입력받는다. 쉼표와 대괄호, 세미콜론을 규칙대로 적용하여 [상품명, 가격, 수량]; 의 형태로 입력받아야한다.
- 모든 예외처리는 IllegalArgumentException 을 발생시키고 [ERROR] 문구를 활용하여 규칙대로 처리한다.
