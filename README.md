...
## 구현할 기능 목록
- [x] 초기자본 세팅 기능 
    - 예외) 초기자본이 10원단위가 아닌경우? 
    - 예외) pick한 수가 동전으로 바뀌지 않은, 나머지 돈보다 큰 경우?
      - 남은 금액에 맞춰 리스트가 변해야한다
      - ex. 600원이 자본인데 pick한게 500이다. 그럼 리스트에선 100, 50, 10만 남아야함.
...
4. 상품정보 출력 기능
    - 보류 - 화면을 벗어나서 출력하는 경우? ==상품이 너무 많은 경우
...
10. 종료 기능
    - 남은 투입금액이 최저값인 상품보다 작다면 종료.
    - 잔돈을 반환할 수 없는 경우 잔돈으로 반환할 수 있는 금액만 반환한다.
      - 반환되지 않은 금액은 자판기에 남는다.


11. 잔돈계산 기능
    - 기능) 남은 자본으로 잔돈 계산 가능한지 판별한다
