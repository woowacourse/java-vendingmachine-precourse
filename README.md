...
- 보유 금액으로 랜덤한 동전으로 생성
- 상품 저장 기능
- 투입 금액이 상품을 살수있는지에 대한 검증 기능
- 사용자 투입 금액 저장 기능
- 사용자 물건 구매 후 잔돈 돌려주기 기능 
  -[x] :투입한 금액이 가장 가격이 낮은 상품보다 작을 때 


- **주의 요구사항**
  
  -[x] : 만약 잔돈을 반환할 수 없는 경우 잔돈으로 반환할 수 있는 금액만 돌려주기 
  -[x] : 지폐를 잔돈으로 반환하는 경우는 존재하지 않는다. 
  -[x] : 잔돈을 돌려줄 때 현재 보유한 최소 개수의 동전으로 잔돈을 돌려준다.(**그리디 알고리즘**)
...
- 사용자 입력
  - 처리 순서
    ...
    - 상품명과 가격, 재고량 입력
      - '[]' 괄호가 없는 경우, 괄호가 짝수가 아닌경우
      - 상품명,가격,수량 의 형식이 아닌 경우
          - 상품명 := 특수문자 또는 숫자일 경우
          - 가격 := 숫자가 아닌 경우 또는 0보다 작은 경우, 10으로 나누어 떨이지지 않는 경우
          - 수량 := 숫자가 아닌 경우 또는 0보다 작은 경우
      - ';' 이 안들어 간 경우
