...
- 입력받은 금액으로 무작위하게 동전을 생성한다.
- 생성한 동전을 각 동전 인스턴스에 넣는다.
- 보유한 동전을 출력한다.
- `[상품명, 가격, 수량]`을 입력받는다.
- `상품명, 가격, 수량`이 들어간 `Juice`인스턴스를 생성한다.
- `투입할 금액`을 입력받는다.
- 투입 금액을 출력한다.
- `구매할 상품명`을 입력받는다.
- 구매할 상품의 가격을 투입 금액에서 뺀다.
- 상품을 구매하면 수량을 1개씩 줄인다.
<br>
### 자판기 종료
- 남은 금액이 상품의 최저 가격보다 적은 경우 종료한다.
- 모든 상품이 소진된 경우 종료한다.
- 자판기가 종료되면 잔돈을 계산하여 돌려준다. (최대한 잔돈으로 돌려줄 수 있을만큼)
<br>
### 예외 처리
잘못된 값을 입력할 경우 `IllegalArgumentException`를 발생시키고 "[ERROR]"로 시작하는 메세지를 출력한다.
<br>
에러 메세지가 출력되면 해당 부분을 다시 입력 받는다.
- 최초 투입, 각각 쥬스의 가격, 손님이 투입한 금액 모두 최소단위는 10원, 양의 정수가 되야한다.
- 재고가 없어서는 안된다.
- 재고로 넣는 쥬스를 양식에 맞게 입력해야한다.
- 주문한 메뉴가 메뉴판에 존재해야한다.
- 주문한 메뉴의 재고가 있어야한다.
- 투입한 돈보다 주문한 메뉴가 더 비싸면 안된다.
