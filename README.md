...
- 자판기가 보유하고 있는 금액을 입력해야 한다.
  - 숫자로만 입력해야 한다.
  - 10원 단위로 나누어 떨어져야 한다.
- 자판기가 보유한 금액을 동전별로 출력해야한다.
- 자판기의 제품을 입력한다.
  - 제품명, 가격, 수량을 입력하되, 개별 제품은 대괄호([])로 묶어 세미콜론(;)으로 구분한다.  
    예)` [콜라,1500,20];[사이다,1000,10]`
  - 사용자가 위의 예시대로 형식에 맞게 입력했는지 확인한다.
  - 제품의 이름이 중복되지 않았는지 확인한다.
  - 제품의 가격, 수량이 숫자로 입력했는지 확인한다.
  - 제품의 가격과 수량이 각 각 0원, 0개보다 큰지 확인한다.
### 제품 구매
- 투입 금액을 입력한다.
  - 숫자로만 입력해야 한다.
  - 10원 단위로 나누어 떨어져야 한다.
- 구매할 제품명을 입력한다.
  - 해당 제품이 존재하는지 확인한다.
  - 해당 제품의 재고가 있는지 확인하다.
  - 금액은 충분히 있지만 사고싶은 제품의 재고가 없을 경우 다른 제품을 입력받도록 한다.
- 해당 제품이 존재하고 재고가 있으면 재고를 하나 감소시킨다.
- 투입한 금액도 제품의 가격만큼 차감한다.
- 구매하고 남은 금액을 출력한다.
### 잔돈 반환
- 남은 금액이 상품의 최저 가격보다 적거나, 모든 상품이 소진된 경우 바로 잔돈을 돌려준다.
  - 잔돈은 자판기가 보유하고 있는 동전으로 돌려줘야 하며, 잔돈으로 반환할 수 있는 금액만 반환한다.
