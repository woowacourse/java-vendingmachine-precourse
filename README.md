...
- 구매할 상품 입력하기
- 남은 투입금액 계산하기
- 종료 조건(남은 금액이 상품의 최저 가격보다 적거나, 모든 상품이 소진된 경우)을 판단
- 잔돈 돌려주기 (최소 개수의 동전으로 잔돈 만들어야함)
...
- 숫자가 아니라 다른 문자를 입력한다.
- 보유하고 있는 금액 입력이 10원 단위가 아니라 1원 단위이다.

1. 상품명과 가격, 수량 입력시
- 괄호 내에 (상품명, 가격, 수량)이 모두 존재하지 않는다.
- 괄호 내가 ","로 구분되어 있지 않는다.
- 가격, 수량이 숫자가 아니다.
- 사용한 괄호가 다르다.
- 세미콜론으로 값을 구분하지 않았다.
- 중복되는 상품명이 존재한다.
- 상품 가격이 100원 미만이다.
- 상품 가격이 10원으로 나누어 떨어지지 않는다.

1. 투입 금액 입력 시
- 숫자가 아니라 다른 문자를 입력한다.
- 투입 금액이 10원으로 나누어 떨어지지 않는다.
