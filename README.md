...
- 자판기가 보유하고 있는 금액 입력
  - 금액 클래스 구현
    - 금액이 숫자인지 확인
    - 금액이 10원으로 나누어 떨어지는지 확인
  - 금액 클래스의 조건에 맞을 때 까지 반복
- 동전을 자동 생성
- 상품명, 가격, 수량을 입력
  - 입력 형식이 "[상품명,가격(숫자),수량(숫자)];..." 인지 확인
    - 상품들 입력의 조건: ';'로 구분, 대괄호로 묶임
    - 상품의 조건:
      - 속성 3개(상품명, 가격, 수량)
      - 가격과 Money 클래스 사용, 수량은 숫자
- 금액 투입
- 상품 구매
  - 상품 목록에 있는지 확인
  - 돈이 충분히 남아있는지 확인
  - 수량이 남아있는지 확인
- 상품 구매가 계속 가능한지 확인
  - 잔액이 상품 가격들 중 하나라도 넘기는지 확인
  - 잔고가 남아 있는 상품이 있는지 확인
- 잔돈 반환
