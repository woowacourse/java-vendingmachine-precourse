...
### 상품명, 가격, 수량
  ...
  - 상품명, 가격, 수량은 ```,``` 로 구분
    - idea : ```split()``` 사용
    - 예외 처리
      - [X] info 길이가 3이 아닐 경우 => format 에 맞지 않은 입력
      - [ ] 상품명, 가격, 수량이 ```,``` 또는 ```공백``` 인 경우
      - [ ] 상품명이 중복되는 경우
         1. 가격이 같다면 수량 합치기
         2. 가격이 다르다면 예외 처리
      - [X] 가격, 수량이 문자인지 확인
  ...
  - 개별 상품은 ```;``` 로 구분
    - idea : ```split()``` 사용
  - [X] 입력된 상품을 상품 객체 배열에 저장
### 투입 금액
  - ```Console.readLine()``` 사용
  - 예외 처리
    1. 숫자인지 확인
    2. 양의 정수인지 확인
  - 상품 구매 시 해당 상품 가격만큼 투입 금액 차감 후 출력
### 자판기가 보유한 동전
  - 보유하는 금액으로 무작위 생성
  - 0개인 동전도 보여줘야 함
### 잔돈
  - 금액 내에서 구매 가능한 상품이 있는지 확인
    - 있다면 투입 금액 차감, 상품 개수 -= 1
    - 없다면 자판기 보유 동전에서 반환 가능한 잔돈 출력
      - 동전 개수를 최소한으로 사용 => 큰 단위부터 사용
    - 프로그램 반복 여부 결정 => idea : boolean 사용
  - 상품이 남아 있는지 확인
    - 있다면 계속해서 거래
    - 없다면 자판기 보유 동전에서 반환 가능한 잔돈 출력
      - 동전 개수를 최소한으로 사용 => 큰 단위부터 사용
