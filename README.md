...
- 자판기가 보유하고 있는 금액을 입력
  - 금액이 32비트 정수가 아니면 예외처리
  - 금액이 음수인 경우 예외처리
  - 금액이 10으로 나누어지지 않으면 예외처리
- 자판기가 보유하고 있는 금액으로 동전을 무작위로 생성
- 무작위로 생성된 동전 출력
- 상품명과 가격, 수량을 입력
  - 상품명이 한글(단일 자음, 모음 제외), 영어 알파벳, 숫자외의 문자가 포함되어 있으면 예외처리
  - 상품 가격, 상품 수량이 32비트 정수가 아니면 예외처리
  - 상품 가격이 100원 미만이거나 10원으로 나누어 떨어지지 않을 경우 예외처리
  - 상품 수량이 0이하인 경우 예외처리
- 투입 금액을 입력
  - 금액이 32비트 정수가 아니면 예외처리
  - 금액이 음수인 경우 예외처리
- (*)투입 금액으로 살 수 있는 상품이 있는지 확인(수량이 1 이상인 상품들의 최저 가격보다 크거나 같아야 함)
  - 잔액을 출력
  - 투입 금액으로 살 수 있는 상품이 있는 경우
    - 사고자 하는 상품명을 입력
      - 존재하지 않는 상품명을 입력하면 예외처리
      - 수량이 없는 상품명을 입력하면 예외처리
      - 투입 금액이 입력한 상품명의 가격보다 낮으면 예외처리
    - 상품을 구매하고 다시 *로 돌아간다.
  - 투입 금액으로 살 수 있는 상품이 없는 경우
    - 보유하고 있는 동전 중 최소 개수의 동전으로 반환
    - 잔돈을 출력(반환된 동전만 출력)
    - 종료
- 전체 게임 컨트롤러 구현
