...
- [x] 잔돈을 표시한다.
  - [x] 사용자의 남은 투입 금액을 기계에 저장된 동전으로 환전해준다.
  - [x] 동전으로 표현 불가능하면, 최대한 가능하게 한다.
  - [x] 동전으로 표현하는 방법이 여러개면 가장 적게 사용하는 방법을 택한다.
  - [x] 사용된 동전이 0개인 경우 그 동전은 표시하지 않는다.



## 예외 요구사항

- [x] 모든 예외는 [ERROR]를 포함한 예외 메시지를 출력한다.