...
class ProductTest {
    ...
    Products generateSellerFromInfos(String infos) {
        return new Products(ProductParser.parse(infos));
    }

    @Test
    void validateBelowPrice() {
        Products service = generateSellerFromInfos("[콜라,1500,100]");
    ...
    void validateNonExistingProduct() {
        Products service = generateSellerFromInfos("[사이다,2000,1];[콜라,1000,2]");
        assertThrows(
            IllegalArgumentException.class,
