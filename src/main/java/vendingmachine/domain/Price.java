...
public class Price {
    ...
    private void validateUnitIsCorrect(int price) {
        if (price % MINIMUM_COIN_VALUE != 0) {
            throw new IllegalArgumentException("해당 상품의 금액은 동전으로 만들 수 없는 단위의 숫자입니다.");
        }
    }

    private void validateItIsPositive(int price) {
        if (price < ZERO) {
            throw new IllegalArgumentException("상품의 금액으로 0 이상의 금액을 입력해주세요.");
        }
    }

    private int validateItIsNumber(String priceInput) {
        if (!priceInput.matches(NUMBER_REGEX)) {
            throw new IllegalArgumentException("상품의 가격은 숫자여야 합니다.");
        }
