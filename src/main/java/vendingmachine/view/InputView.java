...
public class InputView {
    ...
    public int inputMoneyForMakeCoin() {
        while (true) {
            print(Input.valueOf("MONEY_GUIDE_MESSAGE").getMessage());
    ...
    private void validateDigit(String Money) {
        for (int m = 0; m < Money.length(); m++) {
            if (!Character.isDigit(Money.charAt(m))) {
                print(Input.valueOf("MONEY_DIGIT_ERROR_MESSAGE").getMessage());
    ...
    private void validateNull(String Money) {
        if (Money.length() == Condition.valueOf("LENGTH_0").getNumber()) {
            print(Input.valueOf("MONEY_LENGTH_0_ERROR_MESSAGE").getMessage());
            throw new IllegalArgumentException();
        }
    }
