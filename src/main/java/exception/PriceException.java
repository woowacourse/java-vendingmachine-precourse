...
public class PriceException {
	public static int isValidPrice(String rawPrice) {
		try {
			...
			throw new IllegalArgumentException(ERROR_PREFIX + PRICE_NOT_NUMBER);
		}
	}
	...
	private static void isPositive(int price) {
		...
		throw new IllegalArgumentException(ERROR_PREFIX + PRICE_NOT_POSITIVE);
	}

	private static void isCoinValue(int price) {
		if (price % MIN_COIN_UNIT != ZERO) {
			throw new IllegalArgumentException(ERROR_PREFIX + PRICE_NOT_COIN_VALUE);
		}
	}
}
