...
public class ProductPriceValidator {
	...
	private static final int MIN_PRICE_INPUT_LENGTH = 1;
	private static final int MINIMUM_PRICE = 100;
	private static final String ZERO = "0";
	private static final String PRICE_NUMBER_PATTERN = "^[0-9]+$";
	private static final String INPUT_NOTHING_ERROR_MESSAGE = "[ERROR] 입력한 자판기 상품가격이 없다.";
	private static final String NOT_RIGHT_PRICE_NUMBER_ERROR_MESSAGE = "[ERROR] 입력한 자판기 상품가격이 올바른수(양수)가 아니다.";
	private static final String START_PRICE_NUMBER_ERROR_MESSAGE = "[ERROR] 입력한 자판기 상품가격이 0으로 시작한다.";
	...
	public static boolean isValidPrice(String productPrice) {
		if (isValidInputLength(productPrice) && isRightPriceNumber(productPrice) && !isUnderThanMinimumPrice(
			productPrice) && isDivisibleByMinimumPriceUnit(productPrice)) {
			return true;
		}
	...
	private static boolean isValidInputLength(String productPrice) {
		if (productPrice.length() < MIN_PRICE_INPUT_LENGTH) {
			throw new IllegalArgumentException(INPUT_NOTHING_ERROR_MESSAGE);
		}
		return true;
	}

	private static boolean isRightPriceNumber(String productPrice) {
		if (!Pattern.matches(PRICE_NUMBER_PATTERN, productPrice)) {
			throw new IllegalArgumentException(NOT_RIGHT_PRICE_NUMBER_ERROR_MESSAGE);
		}
		if (productPrice.startsWith(ZERO)) {
			throw new IllegalArgumentException(START_PRICE_NUMBER_ERROR_MESSAGE);
		}
		return true;
	}
